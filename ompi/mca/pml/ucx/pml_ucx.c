...
int mca_pml_ucx_init(int enable_mpi_threads)
{
    ...
    rc = mca_common_ucx_init(&mca_pml_ucx_component.pmlm_version);
    if (rc < 0) {
         return rc;
     }
...
static ucp_ep_h mca_pml_ucx_add_proc_common(ompi_proc_t *proc)
{
    ...
    const opal_process_name_t *proc_name = &proc->super.proc_name;
...
{
    return ucp_worker_progress(opal_common_ucx.ucp_worker);
}
                    ...
                    ucp_send_callback_t cb)
{
    ...
    if (OPAL_LIKELY(req == NULL)) {
        ...
        MCA_COMMON_UCX_WAIT_LOOP(req, OPAL_COMMON_UCX_REQUEST_TYPE_UCP,
                                 opal_common_ucx.ucp_worker, "ucx send",
